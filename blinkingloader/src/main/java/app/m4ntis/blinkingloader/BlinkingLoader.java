...
public class BlinkingLoader extends LinearLayout {
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr, int defStyleAttrRes) {
        handleTypedArray(context, attrs, defStyleAttr, defStyleAttrRes);
        checkIndicatorConfig(context);
        createIndicators();
    }

    private void handleTypedArray(Context context, AttributeSet attrs, int defStyleAttr, int defStyleAttrRes) {
        if (attrs == null) {
            return;
        }

        TypedArray typedArray;
        if (defStyleAttrRes != 0) {
            typedArray = context.obtainStyledAttributes(attrs, R.styleable.BlinkingLoader, defStyleAttr, defStyleAttrRes);
        } else {
            typedArray = context.obtainStyledAttributes(attrs, R.styleable.BlinkingLoader);
        }
